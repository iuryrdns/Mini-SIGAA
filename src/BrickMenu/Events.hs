  ...
  case e of
    ...
    _ -> case st ^. telaAtiva of
      ...
      TelaConfirmacaoFimSemestre -> case e of
        VtyEvent (V.EvKey (V.KChar 's') []) -> finalizarFimSemestre st True
        VtyEvent (V.EvKey (V.KChar 'S') []) -> finalizarFimSemestre st True
        VtyEvent (V.EvKey (V.KChar 'n') []) -> finalizarFimSemestre st False
        VtyEvent (V.EvKey (V.KChar 'N') []) -> finalizarFimSemestre st False
        _ -> return ()
...
handleEsc st = case st ^. telaAtiva of
  ...
  _ -> modify $ \s ->
    s
      { _telaAtiva = if getFase (st ^. sistema) `elem` [2, 3] then TelaMatriculas else TelaMenu,
...
handleEnter st = case st ^. telaAtiva of
  TelaInicial -> modify $ \s ->
    let proxTela = if getFase (s ^. sistema) `elem` [2, 3] then TelaMatriculas else TelaMenu
     in s {_telaAtiva = proxTela, _mensagemErro = Nothing}
  ...
  TelaRelatorio -> modify $ \s ->
    let prox = if getFase (s ^. sistema) `elem` [2, 3] then TelaMatriculas else TelaMenu
     in s {_telaAtiva = prox}
  _ -> return ()
...
handleMenuSelection st = case L.listSelectedElement (st ^. listaMenu) of
  ...
  Just (_, "Iniciar Planejamento") -> do
    case iniciarPlanejamento (st ^. sistema) of
        ...
        modify $ \s ->
          s
            { _sistema = novoSistema,
              ...
              _mensagemErro = Just "Planejamento iniciado! Fase de alterações gerais."
            }
  ...
  Just (_, "Encerrar Matrículas") -> do
    let (deferidas, indeferidas) = obterMatriculasProcessadas (st ^. sistema)
    ...
    -- Relatorio Base
    let relatorio =
          "--- RESULTADO DAS MATRÍCULAS (FASE 2) ---\\n\n"
    ...
    let sistemaComMatriculas = (st ^. sistema) {_matriculas = listaDeferidas}

    case abrirPeriodoRematriculas sistemaComMatriculas of
      Left erro -> modify $ \s -> s {_mensagemErro = Just erro}
      Right novoSistema -> do
        liftIO $ salvarSistema novoSistema
        let novasOpcoes = Vec.fromList ["Cadastrar Rematrícula", "Mostrar Rematrículas", "Finalizar Semestre", "Sair"]
        modify $ \s ->
          s
            { _sistema = novoSistema,
              ...
              _listaMenu = L.list MenuPrincipal novasOpcoes 1,
              _mensagemErro = Just "Matrículas encerradas. Iniciando Período de Rematrículas."
            }
  Just (_, "Cadastrar Rematrícula") ->
    modify $ \s -> s {_telaAtiva = TelaCadSolicitacao, _foco = focusRing [EditMatAluno, EditMatTurma]}
  Just (_, "Mostrar Rematrículas") -> do
    let todasSols = _rematriculas (st ^. sistema)
    modify $ \s ->
      s
        { _telaAtiva = TelaListaSolicitacoes,
          _listaMenuSolicitacoes = L.list ListaSolicitacoes (Vec.fromList todasSols) 1
        }
  Just (_, "Finalizar Semestre") -> modify $ \s -> s {_telaAtiva = TelaConfirmacaoFimSemestre}
  Just (_, "Sair") -> halt
  _ -> return ()
...
handleEditorGeneric st n e =
  let campoLens =
        lens
          (\s -> M.findWithDefault (editor n (Just 1) "") n (s ^. formularios))
  ...
  case extrair st of
    ...
    Right novoSistema -> do
      ...
      modify $ \s ->
        s
          { _sistema = novoSistema,
            _telaAtiva = if getFase novoSistema `elem` [2, 3] then TelaMatriculas else TelaMenu,
            _foco = focusRing [],
...
finalizarFimSemestre :: AppState -> Bool -> EventM Name AppState ()
finalizarFimSemestre st salvar = do
  case finalizarSemestre (st ^. sistema) of
    Left erro -> modify $ \s -> s {_mensagemErro = Just erro, _telaAtiva = TelaRelatorio}
    Right novoSistema -> do
      if salvar
        then liftIO $ salvarSistema novoSistema
        else return ()

      let novasOpcoes = Vec.fromList ["Visualizar Relatório Geral", "Iniciar Planejamento", "Sair"]
      modify $ \s ->
        s
          { _sistema = novoSistema,
            _telaAtiva = TelaMenu,
            _listaMenu = L.list MenuPrincipal novasOpcoes 1,
            _mensagemErro = Just "Semestre finalizado. Sistema em modo ocioso."
          }
