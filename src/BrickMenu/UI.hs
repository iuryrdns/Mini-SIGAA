...
drawListaAlunos s = templateUI " Lista de Alunos Cadastrados " $
    let desenhaLinha selecionado aluno = 
            ...
            in estilo $ str info
    in vBox [ vLimit 15 $ checkEmpty (s^.listaMenuAlunos) $ L.renderList desenhaLinha True (s^.listaMenuAlunos)
            , str " "
...
drawListaProfessores s = templateUI " Lista de Professores Cadastrados " $
    let desenhaLinha selecionado professor = 
            ...
            in estilo $ str info
    in vBox [ vLimit 15 $ checkEmpty (s^.listaMenuProfessores) $ L.renderList desenhaLinha True (s^.listaMenuProfessores)
            , str " "
            , hCenter $ str "[Esc] Voltar ao Menu"
            ]
...
drawListaDisciplinas s = templateUI " Lista de Disciplinas " $
    let desenhaLinha selecionado disc = 
            let estilo = if selecionado then withAttr L.listSelectedAttr else id
                ...
                info = cod ++ " - " ++ nome
            in estilo $ str info
    in vBox [ vLimit 15 $ checkEmpty (s^.listaMenuDisciplinas) $ L.renderList desenhaLinha True (s^.listaMenuDisciplinas)
            , str " "
            , hCenter $ str "[Esc] Voltar ao Menu"
            ]
...
drawMatriculaMenu s = 
    let titulo = if null (_matriculas (s^.sistema)) 
                 then " Período de Matrículas (Fase 1) " 
                 else " Período de Rematrículas "
...
drawListaTurmas st = templateUI " Lista de Turmas (Ativas/Pendentes) " $
    ...
    in vBox [ vLimit 15 $ checkEmpty (st^.listaMenuTurmas) $ L.renderList desenhaLinha True (st^.listaMenuTurmas)
            , str " "
            , hCenter $ str "[Esc] Voltar ao Menu"
            ]

drawRelatorio :: AppState -> Widget Name
drawRelatorio st = templateUI " Relatório Geral " $
    vBox [ vLimit 20 $ viewport ScrollRelatorio Vertical content
         , str " "
         , hCenter $ str "[Esc] Voltar | [Setas] Rolar"
         ]
  where
    sis = st^.sistema
    content = vBox [ drawSection "Alunos Cadastrados" (drawAlunosReport sis)
                   , drawSection "Professores Cadastrados" (drawProfessoresReport sis)
                   , drawSection "Disciplinas Cadastradas" (drawDisciplinasReport sis)
                   , drawSection "Turmas (Ativas + Pendentes)" (drawTurmasReport sis)
                   , drawSection "Matrículas Efetivadas" (drawMatriculasReport sis)
                   ]

drawSection :: String -> Widget Name -> Widget Name
drawSection title w = 
    vBox [ padTop (Pad 1) $ withAttr (attrName "border") $ hBorderWithLabel (str title)
...
drawAlunosReport s = 
    ...
    in if null alunos then str "Nenhum aluno cadastrado."
       else vBox $ map mkRow alunos
  where
    mkRow a = str $ show (unMatricula $ A.getMatriculaAluno a) ++ " - " ++ unNome (A.getNomeAluno a) ++ 
                    " (" ++ unCurso (A.getCursoAluno a) ++ ", CRA: " ++ show (unCRA $ A.getCraAluno a) ++ ")"
...
drawProfessoresReport s =
    ...
    in if null profs then str "Nenhum professor cadastrado."
       else vBox $ map mkRow profs
  where
    mkRow p = str $ show (unMatricula $ B.getMatriculaProfessor p) ++ " - " ++ unNome (B.getNomeProfessor p) ++
...
drawDisciplinasReport s =
    ...
    in if null discs then str "Nenhuma disciplina cadastrada."
...
drawTurmasReport s =
    ...
    in if null allTurmas then str "Nenhuma turma cadastrada."
       else vBox $ map (mkRow buscaDisc) allTurmas
  where
    mkRow findDisc t = str $ "T" ++ show (T.getCodigoTurma t) ++ " - " ++ findDisc (T.getDisciplinaTurma t) ++
                             " (" ++ show (T.getHorarioTurma t) ++ " | Sala " ++ T.getSalaTurma t ++ ")"
...
drawMatriculasReport s =
    ...
    in if null matriculas then str "Nenhuma matrícula efetivada."
...
drawListaSolicitacoes s = templateUI " Matrículas Realizadas " $
    let sis = s^.sistema
    ...
    in vBox [ vLimit 15 $ checkEmpty (s^.listaMenuSolicitacoes) $ L.renderList desenhaLinha True (s^.listaMenuSolicitacoes)
            , str " "
            , hCenter $ str "[Esc] Voltar ao Menu"
            ]

drawConfirmacao :: Widget Name
drawConfirmacao = center $
    withAttr (attrName "border") $
    borderWithLabel (str " Confirmação ") $
    padAll 2 $
    vBox [ hCenter $ str "Deseja salvar as alterações antes de sair?"
         , str " "
         , hCenter $ hBox 
